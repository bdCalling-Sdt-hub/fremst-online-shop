...
const updateProduct = async (
  id: Types.ObjectId,
  payload: IProduct & { existingFeaturedImages: string[] },
) => {
  if (
    payload.existingFeaturedImages &&
    payload.existingFeaturedImages.length > 0
  ) {
...
const getAllProduct = async (user: JwtPayload, filters: IProductFilters, paginationOptions: IPaginationOptions) => {
  ...
  const [result, userDetails] = await Promise.all([
  ...
  if(company && user.role === USER_ROLES.COMPANY){
    return await updateSalePriceWithCompanyPrices(result, company, user.role)
  }
...
const getSingleProduct = async (id: Types.ObjectId, user: JwtPayload) => {
  ...
  const [product, employee] = await Promise.all([
    ...
    user.role === USER_ROLES.EMPLOYEE
      ? Employee.findById(user.userId, { company: 1 }).populate<{
  ...
  if (user.role === USER_ROLES.EMPLOYEE && employee?.company) {
    ...
    if (company.availableProducts && company.availableProducts.includes(id)) {
      ...
      const updatedProduct = await updateSalePriceWithCompanyPrices(
        [product],
